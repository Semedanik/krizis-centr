(function ($) {
  ...
  var buildAutoplaySrc = function (src) {
    if (!src) return "";
  ...
  var resetVideoSlide = function ($slide) {
    ...
    var $iframe = $slide.find(".video-embed iframe");
    if ($iframe.length) {
      $iframe.attr("src", "");
    }
    $slide.removeClass("is-playing");
  };
  ...
  var customSlickSlider = function () {
    ...
    $(".slide-fade-2").on(
      ...
      function (event, slick, currentSlide) {
        var $current = $(slick.$slides[currentSlide]).find(".video-slide");
        if ($current.length) {
          resetVideoSlide($current);
        }
      }
    );
  ...
  var initVideoSlides = function () {
    $(".video-slide").each(function () {
      var $slide = $(this);
      var $embed = $slide.find(".video-embed");
      var $iframe = $embed.find("iframe");
      var baseSrc = $embed.data("video-src");

      if ($iframe.length) {
        $iframe.attr("src", "");
      }

      $slide.find(".video-play-trigger").on("click", function (e) {
        e.preventDefault();
        if ($iframe.length && baseSrc) {
          $iframe.attr("src", buildAutoplaySrc(baseSrc));
        }
        $slide.addClass("is-playing");
      });
    });
  };
